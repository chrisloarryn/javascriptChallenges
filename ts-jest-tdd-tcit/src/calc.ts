...
export function listClientsIds(): number[] {
  return [...clients].map(({ id }) => id);
}

// 1 Arreglo con los ids de clientes ordenados por rut
export function listClientsIdsSortByTaxNumber(): number[] {
  return [...clients]
...
export function sortClientsTotalBalances(): string[] {
  return [...clients]
    .map(({ id, name }) => {
      const pricesFromClientOrdered = accounts.filter(
        ({ clientId }) => clientId === id,
      );
      return {
        client: name,
        total: pricesFromClientOrdered.reduce(
...
export function banksRankingByTotalBalance() {
  // CODE HERE
}

// 6 Objeto en que las claves sean los nombres de los bancos y los valores el número de clientes que solo tengan cuentas en ese banco.
export function banksFidelity() {
  // CODE HERE
}

// 7 Objeto en que las claves sean los nombres de los bancos y los valores el id de su cliente con menos dinero.
export function banksPoorClients() {
  // CODE HERE
}
...
export function newClientRanking() {
  // CODE HERE
}
